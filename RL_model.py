...
def train(args, kg, dataset, filename):
    ...
    memory_feature_longtail = ReplayMemoryPER(args.memory_size)  # 50000
    memory_item_longtail = ReplayMemoryPER(args.memory_size)  # 50000
    memory_feature_head = ReplayMemoryPER(args.memory_size)  # 50000
    memory_item_head = ReplayMemoryPER(args.memory_size)  # 50000
    ...
    agent = Agent(device=args.device, memory_feature_longtail=memory_feature_longtail, memory_item_longtail=memory_item_longtail,memory_feature_head=memory_feature_head, memory_item_head=memory_item_head , state_size=args.hidden, action_size=embed.size(1), \
    ...
    for train_step in range(1, args.max_steps+1):
        ...
        loss_value = torch.tensor(0, dtype=torch.float, device=args.device)
        loss_policy = torch.tensor(0, dtype=torch.float, device=args.device)
        for i_episode in tqdm(range(args.sample_times),desc='sampling'):
            ...
            epi_reward = 0
            is_last_turn = False
            newloss_feature, newloss_feature_policy = agent.optimize_model(args.batch_size, args.gamma, 'feature')
            newloss_item, newloss_item_policy = agent.optimize_model(args.batch_size, args.gamma, 'item')
            if newloss_feature is not None:
                loss_value += newloss_feature
            if newloss_item is not None:
                loss_value += newloss_item
            if newloss_feature_policy is not None:
                loss_policy += newloss_feature_policy
            if newloss_item_policy is not None:
                loss_policy += newloss_item_policy
            for t in count():   # user  dialog
                ...
                if action == None:
                    break
                if not args.fix_emb:
                    next_state, next_cand, action_space, reward, done, action_type, recom_items, asked_features = env.step(action.item(), sorted_actions, agent.gcn_net.embedding.weight.data.cpu().detach().numpy())
                else:
                    next_state, next_cand, action_space, reward, done, action_type, recom_items, asked_features = env.step(action.item(), sorted_actions)
                epi_reward += reward
                ...
                if action_type == 0 :
                    agent.memory_feature_longtail.push(state, action, sorted_actions[0][:10], sorted_actions[1][:10], next_state, reward, next_cand)
                else:
                    agent.memory_item_longtail.push(state, action, sorted_actions[0][:10], sorted_actions[1][:10], next_state, reward, next_cand)
                state = next_state
                cand = next_cand
                if done:
        ...
        print('loss_value : {} in epoch_uesr {}'.format(loss_value.item()/args.sample_times, args.sample_times))
        print('loss_policy : {} in epoch_uesr {}'.format(loss_policy.item() / args.sample_times, args.sample_times))
...
def main():
    ...
    parser.add_argument('--observe_num', type=int, default=500, help='the observe_num')
    ...
    parser.add_argument('--test_type', type=str, default='ALL', help='ALL test or Part test')
